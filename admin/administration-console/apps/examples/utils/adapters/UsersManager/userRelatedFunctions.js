...
exports.createUser = function (userData, callback) {
    //Verificarea existentei unui utilizator cu aceeasi adresa de email
    persistence.filter("DefaultUser",{"email":userData.email},function(err,result){
        if(err){
            ...
            //Crearea unui obiect de tip apersistence nou.
            var user = apersistence.createRawObject("DefaultUser",uuid.v1());
            userData.salt = crypto.randomBytes(saltLength).toString('base64');
            //Apelarea functiei de crearea a parolei pe baza salt-ului generat anterior si a parolei in clar.
            hashThisPassword(userData.password,userData.salt,function(err,hashedPassword){
                userData.password = hashedPassword;
                //Inserarea in baza de date a utilizatorului.
                persistence.externalUpdate(user,userData);
                persistence.save(user,function(err,newUser){
                    if(err){
                        callback(err)
                    }else{
                        exports.addUserToZone(user.userId,userData.zoneName,function(err,result){
                            if(err){
                                callback(err);
                            }else{
                                user.zoneName = userData.zoneName;
...
exports.getUsers = function (callback) {
    //Interogarea SQL pentru preluarea utilizatorilor impreuna cu zona de access corespunzatoare.
    connection.query("Select * from DefaultUser JOIN UserZoneMapping on UserZoneMapping.userId = DefaultUser.userId", callback);
};
...
exports.updateUserZone= function(mappingId,zoneName,callback) {
	connection.query("UPDATE UserZoneMapping set zoneName='"+zoneName+"' where mappingId='"+mappingId+"'", callback);
};
...
exports.updateUser = function (userJsonObj, callback) {
    flow.create("update user", {
        begin: function () {
            //Cautarea in baza de date a utilizatorului, cautare facuta pe baza id-ului
        ...
        updateUser: function (err, user) {
            if (err) {
                ...
                if (persistence.isFresh(user)) {
                    ...
                    //Actualizarea informatiilor in baza de date
...
exports.getUserInfo = function (userId, callback) {
    flow.create("retrieve user info", {
        begin: function () {
            //Cautarea utilizatorului in baza de date pe baza id-ului
        ...
        info: function (err, user) {
            if (err) {
                ...
                //Stergerea informatiilor critice din cadrul obiectului
                delete user['__meta'];
                delete user['password'];
                delete user['salt'];

	            persistence.filter("UserZoneMapping", {userId: userId}, function(err, res){
                    if(err){
	                    callback(err, null);
                    }else if(res){
	                    user.zone = res[0].zoneName;
	                    user.mappingId = res[0].mappingId;
