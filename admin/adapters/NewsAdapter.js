...
queryByState = function (state, callback) {
	self.connection.query("select * from News join DefaultUser on DefaultUser.userId = News.author where News.is_active='"+state+"' order by News.date desc", function (err, result) {
		if(!err){
			cleanObject(result);
		}
		callback(err,result);
